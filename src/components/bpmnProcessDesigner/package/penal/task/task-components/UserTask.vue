<template>
  <el-form label-width="120px">
    <el-form-item label="规则类型" prop="candidateStrategy">
      <el-select v-model="userTaskForm.candidateStrategy" clearable style="width: 100%" placeholder="选择类型"
    ...
    <el-form-item v-if="userTaskForm.candidateStrategy == CandidateStrategy.ROLE" label="指定角色" prop="candidateParam">
      <el-select v-model="userTaskForm.candidateParam" clearable multiple style="width: 100%" placeholder="选择角色"
    ...
    " label="指定部门" prop="candidateParam" span="24"> -->
    <!-- <el-tree-select ref="treeRef" v-model="userTaskForm.candidateParam" :data="deptTreeOptions" :props="defaultProps"
        empty-text="加载中，请稍后" multiple node-key="id" show-checkbox @change="updateElementTask" /> -->
    <!-- </el-form-item> -->
    <!-- <el-form-item v-if="userTaskForm.candidateStrategy == CandidateStrategy.POST" label="指定岗位" prop="candidateParam"
      span="24">
      <el-select v-model="userTaskForm.candidateParam" clearable multiple style="width: 100%"
        @change="updateElementTask">
        <el-option v-for="item in postOptions" :key="item.id" :label="item.name" :value="item.id" />
      </el-select>
    </el-form-item> -->
    <el-form-item v-if="userTaskForm.candidateStrategy == CandidateStrategy.USER" label="指定用户" prop="candidateParam"
      span="24">
      <!-- {{ userTaskForm.candidateParam }}
      <el-select v-model="userTaskForm.candidateParam" clearable style="width: 100%" placeholder="选择用户" multiple
        @change="updateElementTask">
        <el-option v-for="item in userOptions" :key="item.id" :label="item.nickname" :value="item.id" />
      </el-select> -->
      <UserSelector :key="userTaskForm.candidateParam" :ids="userTaskForm.candidateParam" @update:ids="(newIds) => {
        userTaskForm.candidateParam = newIds
        updateElementTask()
      }" />
    </el-form-item>
    <!-- <el-form-item v-if="userTaskForm.candidateStrategy === CandidateStrategy.USER_GROUP" label="指定用户组"
      prop="candidateParam">
      <el-select v-model="userTaskForm.candidateParam" clearable multiple style="width: 100%"
        @change="updateElementTask">
    ...
    <el-form-item v-if="userTaskForm.candidateStrategy === CandidateStrategy.FORM_USER" label="表单内用户字段" prop="formUser">
      <el-select v-model="userTaskForm.candidateParam" clearable style="width: 100%" @change="handleFormUserChange">
        <el-option v-for="(item, idx) in userFieldOnFormOptions" :key="idx" :label="item.title" :value="item.field"
          :disabled="!item.required" />
      </el-select>
    </el-form-item>
    <el-form-item v-if="userTaskForm.candidateStrategy === CandidateStrategy.FORM_DEPT_LEADER" label="表单内部门字段"
      prop="formDept">
      <el-select v-model="userTaskForm.candidateParam" clearable style="width: 100%" @change="updateElementTask">
        <el-option v-for="(item, idx) in deptFieldOnFormOptions" :key="idx" :label="item.title" :value="item.field"
          :disabled="!item.required" />
      </el-select>
    </el-form-item> -->
    <!-- <el-form-item v-if="
    ...
    " :label="deptLevelLabel!" prop="deptLevel" span="24">
    ...
    <!-- <el-form-item v-if="userTaskForm.candidateStrategy === CandidateStrategy.EXPRESSION" label="流程表达式"
      prop="candidateParam">
      <el-input type="textarea" v-model="userTaskForm.candidateParam[0]" clearable style="width: 100%"
        @change="updateElementTask" />
      <el-button class="!w-1/1 mt-5px" type="success" preIcon="ep:select" title="选择表达式" size="small"
        @click="openProcessExpressionDialog">选择表达式</el-button>

      <ProcessExpressionDialog ref="processExpressionDialogRef" @select="selectProcessExpression" />
    </el-form-item> -->
    <!-- 选择弹窗 -->
    <!-- <el-form-item label="跳过表达式" prop="skipExpression">
      <el-input type="textarea" v-model="userTaskForm.skipExpression" clearable style="width: 100%"
...
import { inject, ref, computed, toRaw, watch, nextTick, onMounted, onBeforeUnmount } from 'vue'
import { http } from '@/utils'
import UserSelector from '@/components/common/user/UserSelector.vue'
...
onMounted(async () => {
  ...
  for (let i = 3; i < 10; i++) {
    let tmpData = {
      ...tmp,        // 展开原对象属性
      id: i,         // 覆盖id
      nickname: i.toString() // 覆盖nickname
    }
    userOptions.value.push(tmpData)
  }

  userOptions.value.push(tmp)
