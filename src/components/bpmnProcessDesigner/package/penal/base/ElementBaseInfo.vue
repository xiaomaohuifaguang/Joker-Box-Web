    ...
    <el-form label-width="90px" :model="elementBaseInfo" :rules="rules">
      <div v-if="needProps.type == 'bpmn:Process'">
        ...
        <el-form-item label="流程标识" prop="id">
          <el-input v-model="elementBaseInfo.id" placeholder="请输入流标标识" :disabled="true" @change="handleKeyUpdate">
            <template v-if="props.editType !== 'edit'" #append>
              <el-button @click="handleKeyUpdate(`Process_${new Date().getTime()}`)" style="">重置id</el-button>
            </template>
          </el-input>

        </el-form-item>
        <el-form-item label="流程名称" prop="name">
          <el-input v-model="elementBaseInfo.name" placeholder="请输入流程名称" clearable @change="handleNameUpdate" />
        </el-form-item>
      </div>
      <div v-else>
