...
import { logout,alert } from '@/utils';
...
class Http {
  ...
  //   if(options.server) options.url = CONSTANTS.SYSTEM.SERVERS[options.server] + options.url;
  ...
  result(options: RequestOption): Promise<any> {
    if(options.server && options.server != null) options.url = CONSTANTS.SYSTEM.SERVERS[options.server] + options.url;
