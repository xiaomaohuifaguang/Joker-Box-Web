<template>
  <div class="crawler-task-page">
    <PageHeader :icon="Scissor" title="爬虫任务管理" description="管理系统爬虫任务配置" />
    ...
    <div class="page-container">
      ...
      <div class="table-section" v-loading="loading" element-loading-text="加载中...">
        <div class="section-header">
          ...
          <span class="header-title">爬虫任务列表</span>
    ...
    <el-dialog
      ...
      <CrawlerTaskInfoView v-model:id="dialogEdit.id" v-model:type="dialogEdit.type" :key="dialogEdit.id" />
    </el-dialog>
    ...
    <el-dialog
      ...
      destroy-on-close
      @closed="queryPage"
      class="add-crawler-dialog">
      <CrawlerTaskAddView @success="handleAddSuccess" />
    </el-dialog>
  </div>
</template>
...
import { House, Search, Plus, View, Edit, Delete, Scissor, List, Clock, Timer } from '@element-plus/icons-vue'
import { http, alert, confirm } from '@/utils';
import { onMounted, ref } from 'vue';
import CrawlerTaskInfoView from './CrawlerTaskInfoView.vue';
...
const queryPage = async () => {
    loading.value = true
    const result = await http.post('/crawlerTask/queryPage', {
...
const remove = async (id: any) => {
    await http.post('/crawlerTask/remove', { id })
    alert('删除成功', 'success')
    queryPage()
}
...
const confirmDelete = (id: string) => {
    confirm('提示', '确定删除该CrawlerTask吗？', async () => {
        await remove(id)
    })
}
