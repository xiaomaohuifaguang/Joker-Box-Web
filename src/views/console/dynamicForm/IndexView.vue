            ...
            <div v-loading="loading" element-loading-text="加载中..."
                ...
                <el-card class="table-card" shadow="never">
                    <el-table :data="tableData" stripe border style="width: 100%"
                        ...
                        <el-table-column prop="id" label="表单id" min-width="150" />
                        ...
                        <el-table-column label="操作" fixed="right" width="220" align="center">
                            <template #default="scope">
                                ...
                                <el-button type="success" link size="small"
                                    @click="makeUrl(scope.row.id, scope.row.version)" v-if="scope.row.status == '1'">
                                    <span>跳转实例链接</span>
                                </el-button>
                            </template>
                        </el-table-column>
...
const confirmDelete = (id: string) => {
    confirm('提示', '确定删除该DynamicForm吗？', () => {
        remove(id)
    })
}

const makeUrl = (id: string, version: string) => {
    const protocol = window.location.protocol;      // 协议 (http: 或 https:)
    const hostname = window.location.hostname;      // 主机名/域名
