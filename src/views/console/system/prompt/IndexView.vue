<template>
  <div class="prompt-management-page">
    <PageHeader :icon="ChatLineRound" title="系统提示管理" description="管理系统AI提示词配置" />
...
const queryPage = async () => {
    loading.value = true
    const result = await http.post('/systemPrompt/queryPage', {
...
const remove = async (id: any) => {
    await http.post('/systemPrompt/remove', { id })
    alert('删除成功', 'success')
    queryPage()
}
...
const confirmDelete = (id: string) => {
    confirm('提示', '确定删除该SystemPrompt吗？', async () => {
        await remove(id)
    })
}
