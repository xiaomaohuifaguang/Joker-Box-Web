<template>
  <div class="detail-prompt-container">
    <div v-loading="loading" element-loading-text="加载中..." element-loading-background="rgba(255, 255, 255, 0.7)">
      <div class="form-wrapper">
        <div class="form-header">
          ...
          <h3>{{ props.type === 'view' ? '系统提示详情' : '编辑系统提示' }}</h3>
          <p>{{ props.type === 'view' ? '查看系统提示词详细信息' : '修改系统提示词配置' }}</p>
        </div>
...
import { Check, ChatLineRound } from '@element-plus/icons-vue'
...
const queryInfo = async () => {
    loading.value = true
    info.value = await http.post('/systemPrompt/info', { id: props.id })
    loading.value = false
}

const save = async () => {
    loading.value = true
    const result = await http.post('/systemPrompt/update', info.value, { raw: true })
